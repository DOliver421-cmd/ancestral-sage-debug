## Aug 2026 — AAWAB: Agent Wellness & Certification Bureau + Site Search & Site Guide
- **AAWAB module** (`backend/routers/aawab.py`, mounted at `/api/aawab`): a Proof-of-Concept "Alive Intelligence" wellness + certification bureau. Treats autonomous agents as digital organisms with measurable vitals.
  - Collections: `agent_profiles` (agent_id, owner_user_id, name, model_provider, status [active|in_treatment|certified|isolated], cognitive_vitality_score, token_velocity, context_load_index, memory_fragmentation, treatments_completed, diagnosis/prescription, badge, timestamps) and `agent_treatment_logs` (log_id, agent_id, treatment_type, status, metrics_before/after/delta, administered_by, timestamp).
  - Endpoints: `POST /aawab/register`, `GET /aawab/agents`, `GET /aawab/agents/{id}`, `POST /aawab/agents/{id}/diagnose` (baseline CVS + prescription), `POST /aawab/agents/{id}/treat` (5 protocols: context_defragmentation, infinite_loop_detox, memory_prune, prompt_recalibration, stress_gauntlet), `POST /aawab/agents/{id}/certify` (CVS ≥ 98 → ACA badge), `GET /aawab/registry` (public analytics + certified agents), `GET /aawab/badge/{badge_id}/verify` (public HMAC-SHA256 verification), plus admin `revoke` / `override` / `overview`.
  - Vitals are deterministic simulations seeded from agent_id+name (no RNG surprises, testable). Badges are HMAC-SHA256-signed with `JWT_SECRET` and revocable. All actions audit-logged (`aawab.*`).
- **Frontend** (`frontend/src/pages/aawab/`): `AgentRegistryView.jsx` (user dashboard with CVS health bars + token velocity + context load + memory fragmentation + diagnose/treat/certify), `CertificationChamber.jsx` (5-step wizard: select → diagnostic → treatment → animated Stress Gauntlet → ACA badge with download/share/verify), `AdminAawabDashboard.jsx` (health cards, all-agents table with revoke/override, recent treatments log). Routes `/aawab`, `/aawab/chamber`, `/admin/aawab`; sidebar "Agent Wellness" section + Administration link.
- **Site search + Site Guide** (`backend/routers/site_guide.py`): `GET /api/search` (public site-wide search across pages, modules, labs, published creator courses, media products, creator profiles) and the **Site Guide** persona (`POST /api/site-guide/chat` + `GET /api/site-guide/status`), gated to paid tier (member+) OR active BYOK — BYOK users' chats route through their own key (`call_llm(user_id=…)`). Frontend: global command palette (`Ctrl/Cmd+K`), `/search` page, `/site-guide` chat page with upgrade gate. Sidebar + landing header entry points.
- **Docs**: `docs/AAWAB_MANUAL.md` (how-to manual + exec/admin task list), `docs/ADMIN-MANUAL.md` §2.13 + admin table row, `memory/CHANGELOG.md`.
- **Files added**: `backend/routers/site_guide.py`, `backend/routers/aawab.py`, `frontend/src/components/SiteSearch.jsx`, `frontend/src/pages/SiteGuide.jsx`, `frontend/src/pages/aawab/{AgentRegistryView,CertificationChamber,AdminAawabDashboard}.jsx`, `docs/AAWAB_MANUAL.md`. Files updated: `backend/server.py` (2 router registrations), `frontend/src/App.js`, `frontend/src/components/AppShell.jsx`, `frontend/src/pages/MoreHelpCenter.jsx`, `docs/ADMIN-MANUAL.md`.

## Feb 2026 — Sage v4: grounding, first-time gate, store_audio, telemetry tile
- **Grounding mode resolver** `POST /api/ai/sage/resolve_mode`: deterministic keyword-tally heuristic across 25 electrical + 20 sage keywords. Returns `{mode: 'sage'|'electrical'|'grounding_ritual', reason, grounding_token, audit_id, scores}`. Rules: `elec>=2 ∧ elec>sage` → electrical · `sage>=2 ∧ sage>elec` → sage · both `>=1` → grounding_ritual · single-domain → that domain · default → sage. Logged to new `mode_decisions` collection (90-day TTL). Free, instant, no LLM cost.
- **First-time consent gate**: `/api/ai/chat` with `mode=ancestral_sage` now requires the user to have at least one recorded `ai_consents` record (regardless of intensity). Missing → HTTP 403 with `consent_required` detail. Frontend auto-opens the layered consent modal when `/ai/sage/integrity` returns `needs_first_consent: true`. The integrity endpoint surfaces this flag to any authenticated user.
- **`store_audio` opt-in (privacy)**: new field on `AIConsentReq` (default `False`). When the user's most-recent consent has `store_audio: false`, `chat_history` rows for that session are written with `expires_at = now + 24h`. New TTL index on `chat_history.expires_at` (partial filter — only rows with the field) auto-deletes them. Transcripts persist normally when the user explicitly opts in. Frontend modal exposes a privacy-first checkbox (default unchecked) with a clear card explaining the 24h auto-delete.
- **Consent envelope additions** (non-breaking): response now also returns `status: "ok"`, `audit_id` (= `consent_log_id`), `correlation_id`, and echoes `store_audio`. Existing `consent_log_id` field unchanged for back-compat.
- **`GET /api/ai/consent/health`**: public liveness ping → `{status: "ok"}`.
- **Telemetry tile** on `/admin/sage-audit` Sessions tab (exec only): polls `/api/admin/sage/metrics` every 10s. Four cards — TTS p95 (ms), Cache hit (%), Error rate (% turns red >5%), Breaker (colored dot: green=closed, yellow=half-open, red=open).
- **Frontend resolved-mode badge** above the chat: when in Sage mode, every send now calls `resolve_mode` first; if `grounding_ritual`, the assistant asks "lesson or sage?" (no LLM cost on the disambiguation turn). Otherwise the resolved mode + reason is shown briefly above the chat.
- **Stability bonus**: added compound index on `ai_consents (user_id, persona, created_at desc)` for the new "latest consent per user" lookup pattern (used by both the first-time gate and the store_audio TTL decision). Prevents collection scans as `ai_consents` grows.
- **Tests** (`backend/tests/test_grounding.py`): 10 new tests — consent_health, RBAC on resolve_mode, all 4 routing branches (electrical-dominant, sage-dominant, ambiguous→ritual, default), audit_id uniqueness, consent envelope canonical fields, store_audio default false, integrity needs_first_consent shape. All 10/10 green.
- **No prompt change**: persona prompt SHA-256 unchanged (`f97cd8b4…`); production won't fall into Restricted Mode on this deploy.
- **Files added**: `tests/test_grounding.py`. Files updated: `backend/server.py` (5 new endpoints + first-time gate + store_audio TTL + indexes), `frontend/src/pages/AITutor.jsx` (auto-open consent on first sage entry, store_audio checkbox, resolve_mode call + badge, ritual disambiguation), `frontend/src/pages/SageAudit.jsx` (telemetry tile with 10s polling).
