# Morehelp.center Starter Library

This directory contains the seven original ebook manuscripts for the **Morehelp.center Starter Library**. Each manuscript is intentionally plain Markdown so it can be reviewed, converted, and uploaded through the existing media-store upload flow without introducing a second publishing system.

## Products

- **The Small Start** — A Practical Guide to Turning One Good Idea Into Something Real
- **From Creator to Product** — How to Turn Your Writing, Music, Knowledge, and Ideas Into Things People Can Use
- **AI Without the Intimidation** — A Human-First Guide to Using AI Without Losing Your Judgment
- **The Community Funding Starter** — A Practical Guide to Turning a Good Community Idea Into a Fundable Plan
- **When AI Is Wrong** — A Practical Guide to Catching Confident Errors Before They Cost You
- **The Automation Trap** — When Convenience Starts Making the Decisions
- **The Human-Made Difference** — Voice, Culture, and Responsibility in an AI-Assisted World

## Required store setup

The existing `/store` page already reads published records from `GET /api/media/products` and sends paid products to `POST /api/media/products/{id}/checkout`. Upload each Markdown manuscript (or a reviewed PDF/EPUB conversion) through the existing seller flow, set `price_cents` to `400`, `type` to `pdf` or `other`, add the matching cover, and publish it. No new checkout, payment provider, database collection, or storefront is introduced here.

The startup seed creates published platform product records for these manuscripts with a $4.00 price and the existing authenticated media-delivery path. A real production purchase still requires configured payment credentials and an end-to-end checkout test; source seeding alone is not evidence that payment works.
