...
async def add_security_headers(request: Request, call_next):
    ...
    response.headers["Content-Security-Policy"] = (
        ...
        "connect-src 'self' https://us-assets.i.posthog.com https://us.i.posthog.com https://www.youtube-nocookie.com https://app.lemonsqueezy.com https://api.lemonsqueezy.com https://api.nasa.gov https://images-api.nasa.gov https://phuymlvvxxvcejyfxwui.supabase.co; "
...
async def _on_startup_impl():
    ...
    # /api/nam/knowledge endpoints use: existing knowledge_ids are skipped,
    # so pre-existing production knowledge is never overwritten, and only
    # missing corpus items are added. Non-fatal: a failure degrades to a
    # warning, exactly like the other startup seeds.
    try:
        from seed_nam_knowledge import seed_nam_knowledge as _nam_seed

        _nam_result = await _nam_seed()
        logger.info(
            "STARTUP: Hybrid NAM knowledge seed done — %d seeded, %d skipped, %d failed.",
            _nam_result["seeded_count"],
            _nam_result["skipped_count"],
...
async def ai_chat(body: AIChatReq, user: User = Depends(current_user)):
    ...
    try:
        ...
        _llm = await call_llm(
            ...
            max_tokens=2048,
            persona_label=body.mode or "tutor",
            user_id=user.id,
        )
        reply = _llm.get("text", "")
    except Exception as e:
...
async def ai_revenue_director(body: dict, user: User = Depends(current_user)):
    ...
    system  = get_persona("revenue_director") + (
        ...
        f"- OPENAI_API_KEY (DALL-E 3 via Architect): {'SET' if os.environ.get('OPENAI_API_KEY', os.environ.get('EMERGENT_LLM_KEY', '')) else 'NOT SET — visual briefs only'}\n"
    ) + memory_ctx
    ...
    try:
        ...
        _llm = await call_llm(
            ...
            messages=[{"role": "user", "content": message}],
            max_tokens=4096,
            persona_label="revenue_director",
            user_id=user.id,
        )
        reply = _llm.get("text", "")
    except Exception as e:
        logger.exception("Revenue Director AI error")
        raise HTTPException(502, f"AI error: {e}")

    await log_episode(db, session_id, "revenue_director", user.id, message, reply, [])
...
async def sage_create(body: dict, user: User = Depends(current_user)):
    ...
    try:
        ...
        _llm = await call_llm(
            ...
            messages=[{"role": "user", "content": message}],
            max_tokens=4096,
            persona_label="ancestral_sage",
            user_id=user.id,
        )
        reply = _llm.get("text", "")
    except Exception as e:
        logger.exception("Ancestral Sage AI error")
        raise HTTPException(502, f"AI error: {e}")

    await log_episode(db, session_id, "ancestral_sage", user.id, message, reply, [])
...
async def lab_ai_feedback(sub_id: str, user: User = Depends(current_user)):
    ...
    try:
        from ai.llm_gateway import call_llm
        _llm = await call_llm(
            system="You are a supportive lab feedback assistant.",
            messages=[{"role": "user", "content": prompt}],
            max_tokens=400,
            persona_label="lab_feedback",
            user_id=None,
        )
        feedback_text = _llm.get("text", "")
    except Exception as e:
        logger.exception("Lab feedback AI error")
        raise HTTPException(502, f"AI error: {e}")
...
async def ai_ambassador(body: dict, user: User = Depends(current_user)):
    ...
    try:
        ...
        _llm = await call_llm(
            ...
            messages=[{"role": "user", "content": message}],
            max_tokens=4096,
            persona_label="ambassador",
            user_id=user.id,
        )
        reply = _llm.get("text", "")
    except Exception as e:
        logger.exception("Ambassador AI error")
        raise HTTPException(502, f"AI error: {e}")

    await log_episode(db, session_id, "ambassador", user.id, message, reply, [])
...
async def exec_pipeline_process(
    ...
    Analyzer:
        - Free LLM gateway (llm mode)
        - Keyword fallback when gateway is unavailable  (offline mode)
    """
