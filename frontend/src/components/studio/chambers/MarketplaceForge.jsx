...
export default function MarketplaceForge() {
  ...
  return (
    <div style={{ fontFamily: "inherit", color: "rgba(28,25,23,0.9)", display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      {/* Deliverable file */}
      <div style={{ border: "1px solid rgba(194,65,12,0.25)", background: "rgba(194,65,12,0.04)", padding: "12px 14px" }}>
        <div style={labelStyle}>Deliverable file (PDF, WAV, MP4, ZIP…)</div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <label style={{ display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer", background: "rgba(194,65,12,0.12)", border: "1px solid rgba(194,65,12,0.35)", color: "#c2410c", padding: "7px 14px", fontFamily: "monospace", fontSize: 11, fontWeight: 700 }}>
            <Upload style={{ width: 12, height: 12 }} />
            {uploading ? "Uploading…" : "Attach File"}
            <input type="file" hidden onChange={e => uploadFile(e.target.files?.[0])} />
          </label>
          {fileUrl && (
            <span style={{ fontSize: 12, color: "#15803d", fontFamily: "monospace" }}>
              <Check style={{ width: 12, height: 12, verticalAlign: "-2px" }} /> Deliverable attached
            </span>
          )}
          {!fileUrl && (
            <span style={{ fontSize: 11, color: "rgba(28,25,23,0.4)" }}>
      ...
      <button
        ...
        style={{
          ...
          background: forging || uploading ? "rgba(194,65,12,0.3)" : "linear-gradient(135deg, #9a3412, #c2410c)",
          border: "none", color: "#ffffff", fontWeight: 900, fontSize: 13,
      ...
      {/* ── Existing drops ── */}
      <div>
        <div style={{ fontSize: 9, fontFamily: "monospace", letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(194,65,12,0.7)", marginBottom: 10 }}>
        ...
        {drops.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {drops.map(d => (
              <div key={d.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", background: "rgba(28,25,23,0.04)", border: "1px solid rgba(28,25,23,0.07)" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 900, color: d.published ? "rgba(28,25,23,0.9)" : "rgba(28,25,23,0.5)" }}>{d.title}</div>
                  <div style={{ fontSize: 10, fontFamily: "monospace", color: "rgba(28,25,23,0.3)", marginTop: 2 }}>
                ...
                <span style={{
                  ...
                  color: d.published ? "#15803d" : "rgba(28,25,23,0.4)",
                  border: `1px solid ${d.published ? "rgba(21,128,61,0.4)" : "rgba(28,25,23,0.15)"}`,
                }}>
      ...
      <a
        ...
        style={{ fontSize: 11, fontFamily: "monospace", color: "#c2410c", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}
      >
...
const labelStyle = { display: "block", fontSize: 10, fontFamily: "monospace", letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(194,65,12,0.8)", marginBottom: 6 };
const inputStyle = { width: "100%", background: "rgba(28,25,23,0.04)", border: "1px solid rgba(194,65,12,0.25)", padding: "9px 12px", color: "rgba(28,25,23,0.9)", fontSize: 13, fontFamily: "inherit", outline: "none", borderRadius: 4, boxSizing: "border-box" };
