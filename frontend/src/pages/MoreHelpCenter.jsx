...
function ExecPanel({ apiOnline, visibility, setVisibility, superExec }) {
  ...
  const [confirmResetBudget, setConfirmResetBudget] = useState(false);

  const revokeApiKey = async (envKey) => {
    setConfirmRevokeKey(envKey);
  };

  const doRevokeApiKey = async () => {
  ...
  const resetBudgetCounter = async () => {
    setConfirmResetBudget(true);
  };

  const doResetBudgetCounter = async () => {
    setConfirmResetBudget(false);
    try {
